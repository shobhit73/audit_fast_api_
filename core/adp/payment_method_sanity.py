...
# =========================================================
# ADP Payment Method Sanity Check (MCP core port)
#
# Ported verbatim from the Streamlit module
# ../../apps/adp/payment_method_sanity.py (root repo). The ONLY differences
# are I/O shape: this version takes (content: bytes, filename: str) instead of
# a Streamlit file-uploader object, and exposes run_adp_payment_method_sanity()
# returning (xlsx_bytes, csv_bytes, summary_dict) for the MCP handler. The
# distribution-rule logic (_fix_employee and friends) is kept byte-for-byte in
# sync with the Streamlit version -- fix bugs in BOTH places.
#
...
def _read_adp_file(content: bytes, filename: str) -> pd.DataFrame:
...
def run_adp_payment_method_sanity(content: bytes, filename: str = "adp_payment.xlsx"):
    ...
    summary = {
        ...
        "before_after": preview_df.head(PREVIEW_CAP).to_dict("records"),
    }
    if len(preview_df) > PREVIEW_CAP:
        summary["before_after_note"] = (
