...
# ─────────────────────────────────────────────────────────────────────────────
# FastAPI / MCP entry point
# ─────────────────────────────────────────────────────────────────────────────

def _read_content(content) -> pd.DataFrame:
    """Read bytes-or-path-or-file-like into a string DataFrame.

    Matches the convention used by other audit_fast_api/core/adp/*.py
    modules: accept whatever the MCP load_file() returns (bytes for base64
    uploads, path for local files) and produce a dtype=str DataFrame.
    """
    import os
    ...
    if isinstance(content, str) and os.path.isfile(content):
        name = content.lower()
        if name.endswith(".csv"):
            return pd.read_csv(content, dtype=str, keep_default_na=True)
        return pd.read_excel(content, dtype=str, engine="openpyxl")
    # File-like fallback.
    raw = content.getvalue() if hasattr(content, "getvalue") else content.read()
    buf = io.BytesIO(raw)
    try:
        return pd.read_excel(buf, dtype=str, engine="openpyxl")
    except Exception:
...
def _df_to_records(df: pd.DataFrame) -> list:
    """Convert a DataFrame to a list of plain dicts for JSON / Excel writers."""
    if df is None or df.empty:
        return []
    return df.fillna("").astype(str).to_dict(orient="records")


def run_adp_withholding_audit(uzio_content, adp_content) -> dict:
    """MCP-compatible entry point. Returns a dict of {sheet_name: rows}.

    Streamlit's apps/adp/withholding_audit.py is the source of truth; this
    is a thin wrapper that reuses Rohit's pure-function ``run_audit`` and
    flattens the AuditOutput dataclass into the dict shape that
    ``mcp_server.save_results_to_excel`` knows how to write.
    """
    adp_df = _read_content(adp_content)
    uzio_df = _read_content(uzio_content)

    out = run_audit(adp_df, uzio_df, AuditOptions())

    return {
        "Summary": _df_to_records(out.summary_metrics),
        "Field_Mapping_Rules": _df_to_records(out.field_rules),
        "Mismatch_Summary": _df_to_records(out.mismatch_summary),
        "Mismatches_All": _df_to_records(_to_display(out.mismatches)),
        "Mismatches_Active": _df_to_records(
            _to_display(out.mismatches[out.mismatches["EMPLOYMENT_STATUS"] == "ACTIVE"])
            if not out.mismatches.empty else out.mismatches
        ),
        "Mismatches_Terminated": _df_to_records(
            _to_display(out.mismatches[out.mismatches["EMPLOYMENT_STATUS"] == "TERMINATED"])
            if not out.mismatches.empty else out.mismatches
        ),
        "Reciprocity": _df_to_records(out.reciprocity),
        "Stale_UZIO": _df_to_records(out.stale_uzio),
        "Missing_in_UZIO": _df_to_records(out.missing_in_uzio),
        "Missing_in_ADP": _df_to_records(out.missing_in_adp),
        "Employees_with_Mismatches": _df_to_records(out.employees_with_mismatches),
        "W4_History": _df_to_records(out.w4_history),
        "About": _df_to_records(out.about),
    }
